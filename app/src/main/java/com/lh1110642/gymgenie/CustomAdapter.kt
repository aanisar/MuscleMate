...
//            ).show()
            //opens up new scene
            val myIntent: Intent = Intent(view.context,ViewExerciseActivity::class.java).putExtra("muscle", muscel).putExtra("equipment",equipment.text).putExtra("difficulty",difficulty.text).putExtra("type", type).putExtra("description", description).putExtra("name", name.text)
